...
class NeuralSeqRecommender(object):
    def __init__(self, args, n_items, n_users, subgraphs_G, reversed_subgraphs_mapping_i, reversed_subgraphs_mapping_u,
                 reversed_subgraphs_mapping_last_i, sorted_time, n_hyper=2):
        self.args = args
    ...
    def _init_weights(self):
        ...
        for i in self.sorted_time:
            for n in range(self.n_hyper):
                self.all_weights['W_' + str(i) + '_' + str(n)] = tf.Variable(
                    initializer([self.args.emsize, self.args.emsize]), name='W_' + str(i) + '_' + str(n))
    ...
    def _build(self):
        ...
        stacked_features_transformed_input = tf.layers.dense(stacked_features_input, self.args.emsize,
        ...
        self.net = TransformerNet(self.args.emsize, self.args.num_blocks, self.args.num_heads, self.args.seq_len,
                                  dropout_rate=self.dropout, pos_fixed=self.args.pos_fixed, reuse=True)
